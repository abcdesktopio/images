# Dynamic DockerFile application file for abcdesktop.io generated by https://github.com/abcdesktopio/abcdesktopio/oc.apps/make.js
# DO NOT EDIT THIS FILE BY HAND -- YOUR CHANGES WILL BE OVERWRITTEN
# Dockerfile chromium is generated at Tue Aug 25 2026 09:36:06 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64,linux/arm64
#
ARG TAG=4.4
FROM ghcr.io/abcdesktopio/oc.template.ubuntu.gtk.26.04:$TAG
USER root
RUN echo uname=$(uname -m)
RUN if [ $(uname -m) = 'aarch64' ]; then curl -Ls -o /tmp/virtualgl.deb https://github.com/VirtualGL/virtualgl/releases/download/3.1.5/virtualgl_3.1.5_arm64.deb; fi
RUN if [ $(uname -m) = 'x86_64' ];  then curl -Ls -o /tmp/virtualgl.deb https://github.com/VirtualGL/virtualgl/releases/download/3.1.5/virtualgl_3.1.5_amd64.deb; fi
RUN apt-get update && apt-get install --no-install-recommends --yes /tmp/virtualgl.deb && apt-get clean && rm -rf /tmp/virtualgl.deb
# Install package
RUN echo 'debconf debconf/frontend select Noninteractive' | debconf-set-selections
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y chromium libxv1 libglu1-mesa libgl1 libegl1 libxtst6 && apt-get clean && rm -rf /var/lib/apt/lists/* 
# End of install package
LABEL oc.icon="circle_chromium.svg"
LABEL oc.icondata=""
LABEL oc.keyword="chromium,web,browser,internet"
LABEL oc.cat="office"
LABEL oc.desktopfile="chromium-browser.desktop"
LABEL oc.launch="chromium.Chromium"
LABEL oc.template="ghcr.io/abcdesktopio/oc.template.ubuntu.gtk.26.04"
ENV ARGS="--no-default-browser-check --use-gl=angle --use-angle=gl-egl --ignore-gpu-blocklist --enable-gpu-rasterization"
LABEL oc.name="chromium"
LABEL oc.displayname="chromium"
LABEL oc.path="/usr/bin/chromium"
LABEL oc.comment="{{ oc.comment }}"
LABEL oc.type=app
LABEL oc.mimetype="text/html;text/xml;application/xhtml+xml;application/xml;application/rss+xml;application/rdf+xml;video/webm;"
LABEL oc.fileextensions="html;xml;gif"
LABEL oc.legacyfileextensions="html;xml"
LABEL oc.rules="{\"homedir\":{\"default\":true}}"
LABEL oc.acl="{\"permit\":[\"all\"]}"
ENV APPNAME="chromium"
ENV APP="/usr/bin/chromium"
LABEL oc.args="--no-default-browser-check --use-gl=angle --use-angle=gl-egl --ignore-gpu-blocklist --enable-gpu-rasterization"
ENV NVIDIA_VISIBLE_DEVICES=all
ENV NVIDIA_DRIVER_CAPABILITIES=all
ENV LD_PRELOAD=/usr/lib/libdlfaker.so:/usr/lib/libvglfaker.so
ENV VGL_ISACTIVE=1
ENV VGL_DISPLAY=egl0
ENV GEOPROBE_USEGLX=1
ENV PROMAGIC_USEGLX=1
ENV VBOX_CROGL_FORCE_SUPPORTED=1
#
# Permit to create file in directory /var/lib/dbus/
RUN if [ -x /usr/bin/dbus-launch ]; then chmod g+r,g+w,o+r,o+w /var/lib/dbus ; fi

CMD [ "/composer/appli-docker-entrypoint.sh" ]
