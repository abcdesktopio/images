...
# Dockerfile chrome is generated at Mon Aug 31 2026 09:34:14 GMT+0000 (Coordinated Universal Time)
# platforms=linux/amd64
#
ARG TAG=4.4
FROM ghcr.io/abcdesktopio/oc.template.ubuntu.gtk.26.04:$TAG
USER root
RUN curl -fsSL https://dl.google.com/linux/linux_signing_key.pub | gpg --dearmor -o /usr/share/keyrings/google-chrome.gpg
RUN echo 'Types: deb \nURIs: https://dl.google.com/linux/chrome/deb/ \nSuites: stable \nComponents: main \nArchitectures: amd64 \nSigned-By: /usr/share/keyrings/google-chrome.gpg' > /etc/apt/sources.list.d/google-chrome.sources
# Install package
RUN echo 'debconf debconf/frontend select Noninteractive' | debconf-set-selections
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y krb5-user fontconfig fonts-noto fonts-roboto xfonts-100dpi fonts-ubuntu fonts-freefont-ttf dbus-x11 fonts-wine fonts-recommended google-chrome-stable pciutils libgl1 libgl1-mesa-dri libegl1 libgles2 libgl1 libglu1-mesa && apt-get clean && rm -rf /var/lib/apt/lists/* 
...
LABEL oc.template="ghcr.io/abcdesktopio/oc.template.ubuntu.gtk.26.04"
ENV ARGS="--no-first-run --no-default-browser-check --use-gl=angle --use-angle=gl-egl --ignore-gpu-blocklist --enable-gpu-rasterization --disable-dev-shm-usage"
LABEL oc.name="chrome"
LABEL oc.displayname="Chrome"
LABEL oc.path="/usr/bin/google-chrome-stable"
LABEL oc.comment="{{ oc.comment }}"
LABEL oc.type=app
...
LABEL oc.args="--no-first-run --no-default-browser-check --use-gl=angle --use-angle=gl-egl --ignore-gpu-blocklist --enable-gpu-rasterization --disable-dev-shm-usage"
RUN curl -sL --output /tmp/virtualgl.deb https://github.com/VirtualGL/virtualgl/releases/download/3.1.4/virtualgl_3.1.4_amd64.deb && apt-get install -y --no-install-recommends /tmp/virtualgl.deb && rm -f /tmp/virtualgl.deb && apt-get clean
...
ENV PROMAGIC_USEGLX=1
ENV VBOX_CROGL_FORCE_SUPPORTED=1
RUN fc-cache -f -v
#
